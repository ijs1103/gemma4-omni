...
const KakaoIcon = () => (
  <Svg width={20} height={20} viewBox="0 0 24 24" fill="none">
    <Path
      d="M12 3C7.03 3 3 6.36 3 10.5C3 13.03 4.45 15.27 6.68 16.69L5.78 19.94C5.72 20.17 5.97 20.36 6.17 20.23L9.93 17.77C10.6 17.87 11.29 17.92 12 17.92C16.97 17.92 21 14.56 21 10.5C21 6.36 16.97 3 12 3Z"
      fill="#FEE500"
    />
    <Path
      d="M12 3C7.03 3 3 6.36 3 10.5C3 13.03 4.45 15.27 6.68 16.69L5.78 19.94C5.72 20.17 5.97 20.36 6.17 20.23L9.93 17.77C10.6 17.87 11.29 17.92 12 17.92C16.97 17.92 21 14.56 21 10.5C21 6.36 16.97 3 12 3Z"
...
export default function AccountSettingsScreen() {
  ...
  // 연동된 프로바이더 목록에서 첫 번째를 대표 프로바이더로 표시
  const primaryProvider = user?.linkedProviders?.[0];

  // 이메일 마스킹 (앞 2글자 + *** 처리)
  const maskedEmail = user?.email
    ? user.email.replace(/^(.{2})(.*)(@.*)$/, (_m, a, _b, c) => `${a}***${c}`)
    : '—';
      ...
      <View style={styles.section}>
        ...
        <View style={[styles.card, { backgroundColor: cardColor }]}>
              ...
              <View style={styles.row}>
                <View style={styles.rowLeft}>
                  {user.profileImageUrl ? (
                    ...
                    <View style={[styles.profileImagePlaceholder, { backgroundColor: dark ? '#333' : '#E8E8F0' }]}>
                      <Text style={styles.profileImagePlaceholderText}>
                        {user.displayName?.charAt(0).toUpperCase() ?? '?'}
                      </Text>
                    </View>
                  )}
                  <Text style={[styles.rowText, { color: colors.text }]}>{user.displayName}</Text>
                </View>
              </View>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.rowRightText}>{maskedEmail}</Text>
          </View>
          ...
          <View style={styles.row}>
            <View style={styles.rowLeft}>
              {primaryProvider ? ProviderIcon[primaryProvider] ?? <GoogleIcon /> : <GoogleIcon />}
              <Text style={[styles.rowText, { color: colors.text }]}>
                {primaryProvider ? ProviderLabel[primaryProvider] ?? primaryProvider : '소셜 계정'}
              </Text>
            </View>
